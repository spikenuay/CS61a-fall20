...
def announce_highest(who, last_score=0, running_high=0):
    ...
    return say
"""
note 1:在递归使用函数想要传参数给下一个递归函数的时候 
        要定义一个新的更新变量 将要传的值赋给更新的变量
        不能同一个变量名一直递归使用 不然在下一个递归中 会丢失参数
"""
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while num_roll > 0:
        ...
        averaged_max_update = average(num_roll,dice)
        if averaged_max_update > averaged_max:
            averaged_max = averaged_max_update
            num_roll_max = num_roll
        elif averaged_max_update == averaged_max:
            if num_roll < num_roll_max:
                num_roll_max = num_roll
        num_roll -= 1
    return num_roll_max
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn((score_0 + score),opponent_score):
        return 0
